...
export default function DropzoneApp() {
  ...
  const handleFileUpload = async (files) => {
    ...
    for (let file of files) {
      try {
        ...
        if (
          existingKeys.has(normalizedFileName) ||
          batchKeys.has(normalizedFileName)
        ) {
          window.alert(
            `"${file.name}" is already uploaded in this folder or drive.`,
          );
          continue;
        }
        ...
        if (error?.response?.status === 409) {
          window.alert(
            error.response.data?.error ||
              "This file is already uploaded in this folder or drive.",
          );
          continue;
        }
    ...
    try {
      ...
      if (response.data.folder) {
        const newFolder = {
          ...
          type: "folder",
          size: "--",
          date: new Date(response.data.folder.createdAt).toLocaleDateString(
            "en-US",
  ...
  const folderItems = items.filter(
    (item) => item.parentId === (currentFolder ? currentFolder.id : null),
  );
  ...
  const storageUsagePercent = Math.min(
    100,
    (usedStorageGB / storageLimitGB) * 100,
  );
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <aside
        className={`fixed md:static inset-y-0 left-0 z-30 w-72 bg-white border-r border-slate-100 transform transition-transform duration-300 ease-in-out flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)] ${isSidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
      >
        {/* App Title */}
        ...
        <div className="px-6 pb-4 pt-2 flex gap-3">
          <button
            ...
            className="flex-1 flex items-center justify-center bg-white border-2 border-slate-200 rounded-2xl py-3 px-4 shadow-sm hover:border-indigo-600 hover:text-indigo-600 transition-all group font-semibold text-slate-700"
          >
          ...
          <button
            ...
            className="flex items-center justify-center bg-indigo-600 text-white rounded-2xl py-3 px-4 shadow-lg shadow-indigo-200 hover:bg-indigo-700 hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            title="Upload File"
            disabled={uploading}
          >
            {uploading ? (
            ...
            <>
              ...
              {navigationStack.map((folder, idx) => (
                <React.Fragment key={folder.id}>
                  ...
                  <button
                    onClick={() => navigateTo(folder, idx)}
                    className={`hover:text-indigo-600 transition-colors ${idx === navigationStack.length - 1 ? "font-black text-slate-800" : "font-bold"}`}
                  >
                    {folder.name}
                  </button>
                </React.Fragment>
          ...
          {loading ? (
            ...
            <div className="space-y-8">
                    ...
                    >
                      <div className="flex justify-between items-start mb-5">
                        ...
                        <div className="flex gap-1">
                          ...
                          >
                            <Star
                              className={`h-4 w-4 ${item.starred ? "fill-amber-400" : ""}`}
                            />
                          </button>
                          <button
                            onClick={(e) => {
                      ...
                      <h3 className="text-sm font-bold text-slate-800 truncate mb-1.5">
                        {item.name}
                      </h3>
              ...
              {filteredHomeFolders.length === 0 &&
                filteredHomeRecentFiles.length === 0 && (
        ...
        >
          ...
          <div className="flex justify-end gap-3 mt-8">
            ...
            <button
              ...
              className="px-6 py-2.5 text-sm font-bold bg-indigo-600 text-white hover:bg-indigo-700 rounded-xl shadow-md shadow-indigo-200 transition-all"
            >
              Create
            </button>
          </div>
      ...
      {showShareModal && (
        ...
        >
          <p className="text-sm text-slate-500 mb-5 font-medium">
            Anyone with this link can view this {selectedItem?.type}:
          </p>
          <div className="flex items-center gap-2 p-1.5 bg-slate-50 border-2 border-slate-100 rounded-xl">
            <input
              readOnly
              value={getShareUrl(selectedItem)}
              className="flex-1 bg-transparent px-3 py-2 text-sm font-medium text-slate-600 outline-none truncate"
            />
            <button
              onClick={() => copyToClipboard(getShareUrl(selectedItem))}
              className={`p-2.5 rounded-lg font-bold flex items-center transition-all ${copied ? "bg-green-100 text-green-700" : "bg-white border border-slate-200 text-indigo-600 hover:bg-indigo-50 shadow-sm"}`}
            >
              {copied ? (
                <>
          ...
          <button
            ...
            className="w-full mt-8 bg-slate-900 text-white py-3 rounded-xl text-sm font-bold hover:bg-slate-800 transition-colors shadow-lg"
          >
            Done
          </button>
        </ModalWrapper>
        ...
        >
          ...
          <div className="flex justify-end gap-3 mt-8">
            ...
            <button
              ...
              className="px-6 py-2.5 text-sm font-bold bg-red-600 text-white hover:bg-red-700 rounded-xl shadow-md shadow-red-200 transition-all"
            >
              Delete
            </button>
          </div>
...
function NavItem({ icon, label, active, onClick, hasChildren }) {
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center justify-between px-4 py-3 rounded-2xl text-sm font-bold transition-all ${active ? "bg-indigo-50 text-indigo-700" : "text-slate-600 hover:bg-slate-100"}`}
    >
      <div className="flex items-center">
        <span
          className={`mr-3 h-5 w-5 ${active ? "text-indigo-600" : "text-slate-400"}`}
        >
          {icon}
        </span>
        {label}
      ...
      {hasChildren && (
        <ChevronRight
          className={`h-4 w-4 ${active ? "text-indigo-400" : "text-slate-300"}`}
        />
      )}
    </button>
  );
